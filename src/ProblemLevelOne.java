...
public class ProblemLevelOne {
	...
	protected long getMaxProduct(int[] cur, int[][] gridInt, int length) {
		
		long maxPrdt = 0;
		long prdt = 1;
		int gridLength = gridInt.length - 1;
		int wordLength = length - 1;
		...
		if(cur[0] >= wordLength && cur[1] >= wordLength) {
			prdt = 1;
			for(int i=0;i<=wordLength;i++) {
		...
		if(cur[0] < gridLength && cur[1] < gridLength) {
			prdt = 1;
			for(int i=0;i<=wordLength;i++) {
				prdt = prdt * gridInt[cur[0]+i][cur[1]+i];
			}
		...
		if(cur[0] >= wordLength && cur[1] < gridLength) {
			prdt = 1;
			for(int i=0;i<=wordLength;i++) {
				prdt = prdt * gridInt[cur[0]-i][cur[1]+i];
			}
